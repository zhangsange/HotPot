...
package android.support.asy.image;

import java.lang.ref.WeakReference;

import com.hotpot.HotPotApplication;
import com.hotpot.util.Logger;
...
public abstract class ImageWorker {
	...
	protected ImageWorker(Context context) {
		...
		mScreenWidth = UtilsHelper.getScreenWidth((Activity) context);
	}
	...
	public void loadImage(Object data, ImageView imageView) {
		...
		} else if (cancelPotentialWork(data, imageView)) { // 判断这次和imageview绑定的任务是否之前就绑定过，如果绑定过并且是同一个任务返回false，反之取消imageview之前绑定的任务返回true
	...
	private void calculateImageViewHeight(String url, ImageView imageView) {
		String fileName = ((HotPotApplication) ((Activity) mContext)
				.getApplication()).getCacheImageAbsolutePath(url);
	...
	public static boolean cancelPotentialWork(Object data, ImageView imageView) {
		...
		if (bitmapWorkerTask != null) {
			...
			if (bitmapData == null || !bitmapData.equals(data)) {
				bitmapWorkerTask.cancel(true);
				Logger.d(TAG, "cancelPotentialWork - cancelled work for "
						+ data);
			} else {
	...
	private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
		...
		protected Bitmap doInBackground(Object... params) {
			...
			Logger.d(TAG, "doInBackground - finished work");
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			if (bitmap != null && imageView != null) {
				Logger.d(TAG, "onPostExecute - setting bitmap");

				// if (imageViewReference.get() != null
				// && "fullscreen".equals(imageViewReference.get()
				// .getTag())) {
				// calculateImageViewHeight((String) data,
				// imageViewReference.get());
				// }

				if ("fullscreen".equals(imageView.getTag())) {
					calculateImageViewHeight((String) data, imageView);
				}
	...
	protected void clearMemoryInternal() {
		if (mImageCache != null)
